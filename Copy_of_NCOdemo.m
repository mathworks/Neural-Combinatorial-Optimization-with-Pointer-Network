...
%% Hyperparameters
nCities    = 10;        % number of cities (Increase for a bigger problem)
hiddenSize = 32;       % LSTM hidden dimension
...
% Temperature parameter to control softmax sharpness (smaller value is sharper)
temperature = 0.8;
...
for epoch = 1:numEpochs
    ...
    [gradA, lossA] = dlfeval(@actorLoss_AttnUpgrade, actorNet, dlX, adv, tours, entropyCoefficient, temperature);
    % Critic loss
    [gradC, lossC] = dlfeval(@criticLoss, criticNet, encOutAvg, rewards_for_critic);
...
inferenceTemperature = 0.3; % Values < 1.0 make distribution sharper
