...
%% Hyperparameters
numLoc    = 50;         % number of locations 
hiddenSize = 128;       % LSTM hidden dimension (Increase for a bigger problem)
...
% Observe the AvgReward values closely.
...
% Use higher temperature value during training, for exploration
temperature = 1.2;

% Early Stopping Parameters
patiencePercentage = 0.30; 
% Stop if no improvement for patiencePercentage of total epochs
...
layersActor = [
    ...
    % Add embedding
    fullyConnectedLayer(hiddenSize, 'Name', 'fcEmb')
    ...
    % For Key projection
    fullyConnectedLayer(hiddenSize,'Name','fcKey')
    % For Value projection - Not used in current decoder, but keep for potential use
    fullyConnectedLayer(hiddenSize,'Name','fcValue')
    % For Query projection
    fullyConnectedLayer(hiddenSize,'Name','fcQuery')
...
% Initialize network with dummy data to infer sizes 
dummyX = dlarray(rand(2, batchSize, numLoc,  'single'), 'CBT');
...
layersCritic = [
    ...
    fullyConnectedLayer(hiddenSize*2,'Name','cFC1')
    reluLayer('Name','cRelu1')
    fullyConnectedLayer(hiddenSize,'Name','cFC2')
...
for i = 1:preTrainSamples
    if mod(i, 3) == 0
        % Strategy 1: Random start location
        startCity = randi(numLoc);
    elseif mod(i, 3) == 1
...
fprintf('Starting reinforcement learning after pre-training (numLoc=%d, hiddenSize=%d, batchSize=%d)...\n', ...
    numLoc, hiddenSize, batchSize);
...
for epoch = 1:numEpochs
    % Generate random TSP batch [B, N, C] -> [batchSize, numLoc, 2]
    coords = rand(batchSize, numLoc, 2, 'single');
    % Permute to [C, B, T] -> [2, batchSize, numLoc] for dlarray
    ...
    % Get encoder LSTM outputs [Hidden, Batch, Time] = [hiddenSize, batchSize, numLoc]
    encLSTM_out = forward(actorNet, dlX, 'Outputs', 'encLSTM');
    % Average over Time dimension (dim 3) -> [hiddenSize, batchSize, 1]
...
% than the number used during training!
numLoc = 50;
% ====================================================================

% Location Coordinates
coordsTest = rand(1, numLoc, 2, 'single'); % Single instance, B=1
% Permute to [C, B, T] -> [2, 1, numLoc]
...
% Controls softmax sharpness (1.0=standard, <1.0=sharper, >1.0=smoother)
% <1.0 means more exploitation, >1.0 means more exploration
% Use lower temperature value during inference, for exploitation
...
%% Add 2-opt Heuristic Comparison
fprintf('Running 2-opt heuristic...\n');
coordsTestMatrix = squeeze(double(coordsTest)); % Get Nx2 matrix for heuristic

startCityIndex = tourPred(1); % Get the first location from RL tour

[tourHeuristic, lenHeuristic] = solveTSP_2opt(coordsTestMatrix, startCityIndex);
...
%% 5) Visualize NCO Predicted Tour and the Heuristic Tour
...
% Location points
plot(coordsTestMatrix(:,1), coordsTestMatrix(:,2), 'ro', 'MarkerSize', 4, ...
...
plot(coordsTestMatrix(startCityIndex,1), coordsTestMatrix(startCityIndex,2), ...
    'ks', 'MarkerSize', 6, 'MarkerFaceColor', [0.8 0.8 0.8], ...
    'DisplayName', 'Start');
...
sgtitle( sprintf('NCO solver vs 2-opt solution: \\Delta_{length} = %.2f%%', optimalityGap), 'Interpreter','tex');
% END PLOT CODE
...
N = size(dlX,3); % Number of locations (it acts as Time steps in the LSTM)
hiddenSize = net.Layers(strcmp({net.Layers.Name},'encLSTM')).NumHiddenUnits;

% 1. Encoder Pass
% encLSTM_out shape [Hidden, Batch, Time] = [hiddenSize, batchSize, numLoc]
...
% Input encLSTM_out [hiddenSize, batchSize, numLoc], Output keys [hiddenSize, batchSize, numLoc] ('CBT' assumed by fullyconnect)
keys = fullyconnect(encLSTM_out, Wk, Bk);

% Initialize Decoder State
decH = dlarray(zeros(hiddenSize, B, 'like', dlX), 'CB'); % [hiddenSize, batchSize]
decC = dlarray(zeros(hiddenSize, B, 'like', dlX), 'CB'); % [hiddenSize, batchSize]

% Extract DECODER LSTM Weights
decLSTMLayer = net.Layers(strcmp({net.Layers.Name},'decLSTM_params'));
W_dec_lstm = decLSTMLayer.InputWeights;    % [4*hiddenSize, hiddenSize]
R_dec_lstm = decLSTMLayer.RecurrentWeights;% [4*hiddenSize, hiddenSize]
b_dec_lstm = decLSTMLayer.Bias;            % [4*hiddenSize, 1]

% Initialize Decoder Loop Variables
mask = false(1, B, N); % Mask for visited locations [1, Batch, Time/locations]
tours = zeros(B, N, 'uint32'); % Output tours [Batch, Locations]
logProbs = zeros(B, N, 'like', dlX); % Output logProbs [Batch, Locations]
entropy_steps = zeros(1, B, N, 'like', dlX); % Store step entropy [1, Batch, Locations]

% Initial decoder input: Zeros [Hidden, Batch, SeqLen=1] -> [hiddenSize, batchSize, 1] ('CBT')
...
for t = 1:N
    % Decoder LSTM Step
    % Input decIn [hiddenSize, B, 1], decH/decC [hiddenSize, B] -> Output decOut [hiddenSize, B, 1]
    ...
    % Input decOut [hiddenSize, B, 1] -> query_input [hiddenSize, B]
    query_input = reshape(decOut, hiddenSize, B);
    % query [hiddenSize, B]
    query = fullyconnect(query_input, Wq, Bq, "DataFormat","CB");

    % Calculate Scores
    % keys [hiddenSize, B, N=numLoc]
    % query [hiddenSize, B] -> reshape to [hiddenSize, B, 1] for broadcasting
    reshaped_query = reshape(query, hiddenSize, B, 1);
    % scores = sum(keys .* reshaped_query, 1); -> [1, B, N=numLoc]
    ...
    % Apply softmax over the Locations dimension (dim 3)
    % stripdims removes CBT label, DataFormat ensures softmax over N=numLoc
    ...
    % Calculate linear indices into probs [1, B, N] for the chosen locations idx [B, 1]
    ...
    % Calculate Entropy
    % Sum probs.*log(probs) over the Locations dimension (dim 3) -> [1, B, 1]
...
N = size(dlX,3); % Number of locations (Time steps)

hiddenSize = net.Layers(strcmp({net.Layers.Name},'encLSTM')).NumHiddenUnits;

% 1. Encoder Pass
% encLSTM_out shape [Hidden, B, Time] = [hiddenSize, 1, numLoc]
[encLSTM_out, ~] = forward(net, dlX, 'Outputs', 'encLSTM');
...
% Pre-calculate Keys
% Input encLSTM_out [hiddenSize, 1, numLoc], Output keys [hiddenSize, 1, numLoc]
keys = fullyconnect(encLSTM_out, Wk, Bk);

% Initialize Decoder State
decH = dlarray(zeros(hiddenSize, B, 'like', dlX), 'CB'); % [hiddenSize, 1]
decC = dlarray(zeros(hiddenSize, B, 'like', dlX), 'CB'); % [hiddenSize, 1]
...
% Initialize Decoder Loop Variables
mask = false(1, B, N); % [1, 1, numLoc]
tours = zeros(B, N, 'uint32'); % [1, numLoc]
% Initial decoder input [hiddenSize, 1, 1] ('CBT')
decIn = dlarray(zeros(hiddenSize, B, 1, 'like', dlX), 'CBT');
...
for t = 1:N
    % Decoder LSTM Step
    % Input decIn [hiddenSize, 1, 1], decH/decC [hiddenSize, 1] -> Output decOut [hiddenSize, 1, 1]
    [decOut, decH, decC] = lstm(decIn, decH, decC, W_dec_lstm, R_dec_lstm, b_dec_lstm);

    % Attention Mechanism
    % Project Query
    query_input = reshape(decOut, hiddenSize, B); % [hiddenSize, 1]
    query = fullyconnect(query_input, Wq, Bq, "DataFormat","CB"); % [hiddenSize, 1]

    % Calculate Scores
    % keys [hiddenSize, 1, numLoc]
    % query [hiddenSize, 1] -> reshape to [hiddenSize, 1, 1]
    reshaped_query = reshape(query, hiddenSize, B, 1);
    % scores = sum(keys .* reshaped_query, 1); -> [1, 1, numLoc]
    scores = sum(keys .* reshaped_query, 1);
    scores = scores / scaleFactor; % Scale
    ...
    tours(:,t) = idx; % Store scalar idx in tours [1, numLoc]

    % Update Mask
    % Need index for mask [1, 1, numLoc] corresponding to (1, 1, idx)
    linearIndicesMask = sub2ind(size(mask), 1, 1, double(idx));
    mask(linearIndicesMask) = true;

    % Prepare Input for Next Decoder Step
    % Select features from encLSTM_out [hiddenSize, 1, numLoc] using idx
    % Need features for (:, 1, idx)
    linearIndicesEnc = sub2ind([B, N], 1, double(idx)); % Index into dimensions 2 & 3
    featuresSelected = encLSTM_out(:, linearIndicesEnc); % -> [hiddenSize, 1]
    % Reshape to [hiddenSize, 1, 1] ('CBT')
    decIn = dlarray(reshape(featuresSelected, hiddenSize, B, 1), 'CBT');
...
entropyLoss = -mean(entropy) ; % Average entropy over batch
...
entropy_steps = zeros(1, B, N, 'like', dlX); % [1, B, N] Store step entropy [1, Batch, Locations]

% Initial decoder input: Zeros [Hidden, Batch, SeqLen=1] -> [hiddenSize, batchSize, 1] ('CBT')
decIn = dlarray(zeros(hiddenSize, B, 1, 'like', dlX), 'CBT'); % [hiddenSize, B, 1]
...
B = size(dlX, 2);          % Batch size
N = size(dlX, 3);          % Number of locations
hiddenSize = net.Layers(strcmp({net.Layers.Name}, 'encLSTM')).NumHiddenUnits;
...
for t = 1:N
    ...
    % Apply mask for already visited locations
    scores(mask) = -inf;
    ...
    % Ensure masked locations stay masked after normalization
    scores(mask) = -inf;
...
% Reference: https://www.mathworks.com/matlabcentral/fileexchange/169161-fast-2-opt-travelling-salesman-problem-tsp
%
%   [bestTour, bestLength] = solveTSP_2opt(coords)
%   Inputs:
%       coords - Nx2 matrix of location coordinates.
%   Outputs:
%       bestTour - Row vector representing the order of location indices (1 to N).
%       bestLength - The length of the best tour found.

% Add startCity parameter (optional with default of 1)
if nargin < 2
    startCity = 1; % Default start location
end
...
tour = zeros(1, n);
visited = false(1, n);
currentCity = startCity; % Start at specified location
tour(1) = currentCity;
